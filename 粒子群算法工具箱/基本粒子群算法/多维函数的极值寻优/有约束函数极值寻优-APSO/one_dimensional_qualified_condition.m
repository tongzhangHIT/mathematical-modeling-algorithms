...
Lb=[0.1 0.1  0.1  0.1]; %下边界---------可进行更改
Ub=[2.0 10.0 10.0 2.0]; %上边界---------可进行更改
% 默认参数
para=[25 150 0.95]; %[粒子数，迭代次数，gama参数]---------可进行更改
...
%% 目标函数
function f=cost(x)
f=1.10471*x(1)^2*x(2)+0.04811*x(3)*x(4)*(14.0+x(2));%---------可进行更改

% 非线性约束
function [g,geq]=constraint(x)
% 不等式限制条件
Q=6000*(14+x(2)/2);%---------可进行更改
D=sqrt(x(2)^2/4+(x(1)+x(3))^2/4);%---------可进行更改
J=2*(x(1)*x(2)*sqrt(2)*(x(2)^2/12+(x(1)+x(3))^2/4));%---------可进行更改
alpha=6000/(sqrt(2)*x(1)*x(2));%---------可进行更改
beta=Q*D/J;%---------可进行更改
tau=sqrt(alpha^2+2*alpha*beta*x(2)/(2*D)+beta^2);%---------可进行更改
sigma=504000/(x(4)*x(3)^2);%---------可进行更改
delta=65856000/(30*10^6*x(4)*x(3)^3);%---------可进行更改
F=4.013*(30*10^6)/196*sqrt(x(3)^2*x(4)^6/36)*(1-x(3)*sqrt(30/48)/28);%---------可进行更改
%---------可进行更改
g(1)=tau-13600;%---------可进行更改
g(2)=sigma-30000;%---------可进行更改
g(3)=x(1)-x(4);%---------可进行更改
g(4)=0.10471*x(1)^2+0.04811*x(3)*x(4)*(14+x(2))-5.0;%---------可进行更改
g(5)=0.125-x(1);%---------可进行更改
g(6)=delta-0.25;%---------可进行更改
g(7)=6000-F;%---------可进行更改
% 如果没有等式约束，则置geq=[]；这里g(i)是不等式约束，geq为等式约束
geq=[];

%%  APSO Solver
function [gbest,fbest]=pso_mincon(fhandle,fnonlin,Lb,Ub,para)
if nargin<=4,
    para=[20 150 0.95];%---------可进行更改
end
n=para(1);% 粒子种群大小
