...
%% 算法基本参数           
sizepop=50;  %可改         % 种群规模
overbest=10; %可改         % 记忆库容量
MAXGEN=100;  %可改          % 迭代次数
pcross=0.5;  %可改         % 交叉概率
pmutation=0.4; %可改       % 变异概率
ps=0.95; %可改             % 多样性评价参数
length=6;  %必改           % 配送中心数
M=sizepop+overbest;
...
load('city_coordinate.mat');
  carge=[20,90,90,60,70,70,40,90,90,70,60,40,40,40,20,80,90,70,100,50,50,50,80,70,80,40,40,60,70,50,30,70,10,60];
%找出最近配送点
dis=distancematrix(city_coordinate);
for i=1:34
    distance(i,:)=dis(i,bestchrom);
