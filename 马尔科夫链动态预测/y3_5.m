...
for j=1:42
    for i=1:38
       a(j,i)=(x(j,i+1)-x(j,i))/x(j,i); % 求增长率----根据实际情况需要
    end
end

b=zeros(42,38); %建立数据存储，与数据的大小相同
c=zeros(42,25); %建立状态存储，与类别数和状态的平方相同

% 分别为不同增长率赋值--根据增长率的不同定义5种状态
for j=1:42
    for i=1:38
       if  a(j,i)>=0.03
           b(j,i)=2;
