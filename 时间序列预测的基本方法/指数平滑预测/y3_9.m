...
for i=2:n
    st1(i)=alpha*yt(i)+(1-alpha)*st1(i-1);%二次指数预测
    st2(i)=alpha*st1(i)+(1-alpha)*st2(i-1);
end
a=2*st1-st2;
b=alpha/(1-alpha)*(st1-st2);       %参数由公式计算得出，无更改
yhat=a+b;
yhat=yhat';
...
for i=2:n
    st1(i)=alpha*yt(i)+(1-alpha)*st1(i-1);  %进行三次指数预测
    st2(i)=alpha*st1(i)+(1-alpha)*st2(i-1);
...
c=0.5*alpha^2/(1-alpha)^2*(st1-2*st2+st3);  %参数值由公式确定
yhat=a+b+c;
yhat=yhat';
